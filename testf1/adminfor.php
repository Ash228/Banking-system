...
$adminid     = mysqli_real_escape_string($db, $_POST['uname']);
$bdate     = mysqli_real_escape_string($db, $_POST['bdate']);
$password  = mysqli_real_escape_string($db, $_POST['psw']);
$password1 = mysqli_real_escape_string($db, $_POST['psw1']);

$result = mysqli_query($db, "SELECT bdate FROM admin WHERE adminid=$adminid") or die('SQL Error: ' . mysqli_error($db));
...
if ($dba == $bdate) {
    if ($password == $password1) {
        ...
        $sql = "UPDATE employee set password='$password' where adminid=$adminid;";
        if ($stmti = mysqli_prepare($db, $sql)) {
            mysqli_stmt_bind_param($stmti, "si", $password, $adminid);
